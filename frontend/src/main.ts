...
function shell(content: string, page = 'game'): void {
  app.innerHTML = `
    ...
    <footer><p>Made for shared rooms, not profiles. <a href="/privacy" data-nav>Privacy</a> · <a href="/terms" data-nav>Terms</a></p><p>Original AI-assisted artwork · Living Room Lobby is independent.</p></footer>
...
function renderHome(): void {
  ...
  shell(`
    <section class="hero">
      <div class="hero-copy">
        <p class="eyebrow">No console. No pile of phones.</p>
        <h1>Everyone gets<br><em>a turn.</em></h1>
        <p class="lede">Language-light party games that open right in your TV browser. Little kids can share. Grown-ups can just point and play.</p>
        <div class="hero-actions">
          <button class="button primary" id="host-room">Start on this screen <span aria-hidden="true">→</span></button>
          <button class="button secondary" id="show-join">Join a room</button>
        </div>
        <p class="remote-hint"><kbd>↑</kbd><kbd>↓</kbd><kbd>OK</kbd> TV remote friendly</p>
      </div>
    ...
    <section class="join-panel ${joinCode ? 'open' : ''}" id="join-panel" aria-labelledby="join-title">
      <div><p class="step-number">01 / JOIN</p><h2 id="join-title">Enter the four marks</h2><p>Use the code shown on the TV. No account needed.</p></div>
      <form id="join-form">
    ...
    <section class="how" aria-labelledby="how-title"><p class="eyebrow">Built around the sofa</p><h2 id="how-title">Three ways in. Nobody sits out.</h2>
...
function bindCommon(): void {
  document.querySelectorAll<HTMLAnchorElement>('[data-nav]').forEach((link) => link.addEventListener('click', (event) => { if (link.origin !== location.origin) return; event.preventDefault(); history.pushState({}, '', link.href); render(); window.scrollTo(0, 0); }));
}

function leaveRoom(): void { window.clearInterval(polling); room = null; saveSession(null); history.replaceState({}, '', '/'); render(); }
